...
const Navbar = () => {
  ...
  return (
      <nav className="bg-[#24293A] sticky top-0 z-50 ">
        <div className="max-w-screen-2xl mx-auto">
          ...
          <span className="text-xl font-bold text-white bg-[#24293A] rounded-lg border-l border-r-4 border-t border-b-4 border-[#191C27] px-[11px] py-[7px]">
            Oricon
          </span>
              </a>
              ...
              <div className="hidden md:flex items-center space-x-8">
                {navItems.map((item) => (
                    <a
                        key={item.name}
                        href={item.href}
                        className="flex items-center gap-2 text-lg font-regular text-white hover:text-[#2466F4] transition-colors duration-300"
                    >
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="hidden md:flex relative group">
                <button className="text-white border-b border-gray-400 pb-0.5">
                  {currentLanguage}
                </button>
                <div className="absolute right-0 top-full mt-1 text-white bg-[#2E3448] shadow-lg rounded-lg py-2 w-28 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                  {languages.map(lang => (
                      <button
                          key={lang}
                          onClick={() => changeLanguage(lang)}
                          className={`w-full text-left px-4 py-2 hover:bg-[#393F53] ${currentLanguage === lang ? 'font-semibold' : ''}`}
                      >
                        {lang}
                      </button>
                  ))}
